...
function calcM(trades) {
  ...
  trades.forEach(t => {
    ...
    cumPnl += t.pnl;
    let ed = t.exit_date.replace(' (End)', '');
    if (!currentPeakDate) {
      currentPeakDate = ed;
      currentPeakVal = cumPnl;
    } else if (cumPnl >= currentPeakVal) {
      let dur = Math.max(0, Math.floor((new Date(ed) - new Date(currentPeakDate)) / (1000 * 60 * 60 * 24)));
  ...
  if (currentPeakDate && cumPnl < currentPeakVal && trades.length > 0) {
    let lastDt = trades[trades.length - 1].exit_date.replace(' (End)', '');
    let dur = Math.max(0, Math.floor((new Date(lastDt) - new Date(currentPeakDate)) / (1000 * 60 * 60 * 24)));
    if (dur > 0) durations.push(dur);
  }

  let maxDDDur = durations.length ? Math.max(...durations) : 0;
  let avgDDDur = durations.length ? Math.round(s(durations.map(x => ({ d: x })), "d") / durations.length) : 0;
  ...
  return {
    ...
    max_drawdown_points: +maxDD.toFixed(0),
    max_drawdown_duration: maxDDDur,
    avg_drawdown_duration: avgDDDur,
    avg_days_held: +(s(trades, "days_held") / trades.length).toFixed(1),
        ...
        {activeTab === "overview" && m && !loading && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(148px,1fr))", gap: 10, marginBottom: 18 }}>
              ...
              <StatCard label="Max DD Dur" value={m.max_drawdown_duration + "d"} variant="neg" />
              <StatCard label="Avg DD Dur" value={m.avg_drawdown_duration + "d"} variant="neg" />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {filteredTrades.map(t => {
                    const pc = t.pnl_points >= 0 ? T.teal : T.red, cc = t.runCum >= 0 ? T.teal : T.red; return (
          ...
          <div style={cardSt}><SecTitle>Parameter Sweep — Robustness Test</SecTitle>
            ...
            {sweepRows.length > 0 && (
              <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr><th style={thSt}></th>{["Rounding", "Offset", "Trades", "Win Rate", "Profit Factor", "Total P&L pts", "Expectancy", "Max DD", "Max DD Dur", "Avg DD Dur", "Avg Days"].map(h => <th key={h} style={thSt}>{h}</th>)}</tr></thead>
                <tbody>{sweepRows.map((r, i) => {
                  const pfNum = r.profit_factor === "∞" ? 999 : +r.profit_factor, pfColor = pfNum >= 1.5 ? T.teal : pfNum >= 1 ? T.accent : T.red; return (
                    <tr key={`${r.rounding}-${r.offset}`} style={{ borderBottom: `1px solid ${T.border}22`, background: i === 0 ? `${T.accent}08` : "transparent" }}>
                      ...
                      <td style={tdSt}>{r.avg_drawdown_duration}d</td>
                    ...
                    {Object.entries(periods).map(([period, entries]) => {
                      ...
                      return (<div key={pk} style={{ border: `1px solid ${T.border}`, borderRadius: 8, overflow: 'hidden', marginBottom: 8 }}>
                        ...
                        {pOpen && <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead><tr>{['Rounding', 'Offset', 'Start', 'End', 'Trades', 'Win Rate', 'PF', 'Total P&L', 'Expectancy', 'Max DD', 'Max DD Dur', 'Avg DD Dur', 'Avg Days', ''].map(h => <th key={h} style={thSt}>{h}</th>)}</tr></thead>
                          <tbody>{entries.map(e => {
                            ...
                            return (<tr key={e.id} style={{ borderBottom: `1px solid ${T.border}22` }}>
                              ...
                              <td style={tdSt}>{e.max_drawdown_duration !== undefined ? e.max_drawdown_duration : e.max_dd_duration || 0}d</td>
                              <td style={tdSt}>{e.avg_drawdown_duration !== undefined ? e.max_drawdown_duration : e.avg_dd_duration || 0}d</td>
